      ...
      hidden(div(
        ...
        radioButtons(
          ...
          c(
            ...
            "Yes, people with bachelor or higher degree have consistently higher median annual earning" = "college"
          ),
        )
      )),
...
server <- function(input, output, session) {
   ...
   drawPlot <- reactive({
    ...
    g <-
      ggplotly(thePlot,
               tooltip = "text",
               width = plotWidth,
               height = plotHeight)  %>% event_register("plotly_click")
    hide_legend(g)
  })
  ...
  ################################ Functions For 2nd Page ####################################################
   ...
   drawSctLinePlot <- reactive({
      ...
      eventClickData <- event_data("plotly_click")
      d_doubleClick <- event_data("plotly_doubleclick")
      
      print(eventClickData)
      print(d_doubleClick)
      
      clickedYear <- eventClickData["x"]
      if (is.null(eventClickData) | currentLevel() != 2) {
        return(ggplotly(thePlot,width = 1000, height = linePlotHeight(), tooltip = "text")%>% event_register("plotly_click"))
      }
      ...
      if (clickedYear > maxYear | clickedYear < minYear) {
        return(ggplotly(thePlot,width = 1000, height = linePlotHeight(), tooltip = "text") %>% event_register("plotly_click"))
      }
      ...
      if (show_snd_type()) {
        ...
        if (!is.na(oneIncome) && !is.na(sndIncome) & (oneIncome != sndIncome)) {
          ...
          # )
            ...
            ggplotly(thePlot, width=1000, height = linePlotHeight(), tooltip = "text") %>% event_register("plotly_click")
        } else {
          ggplotly(thePlot, width=1000, height = linePlotHeight(), tooltip = "text") %>% event_register("plotly_click")
        }
      }
   ...
   plot2groupBar <- reactive({ 
     eventClickData <- event_data("plotly_click")
     eventDoubleClickData <- event_data("plotly_doubleclick")
     minYear = input$tgp_range[1]
     maxYear = input$tgp_range[2]
    ...
    if (shouldHighlight) {
      ...
      ggplotly(g, width = histBarPlotWidth(), height = histPlotHeight(), tooltip = "text", hoverinfo = "none")  %>% event_register("plotly_click")
    } else {
      if (gender() == snd_gender() && data_type() == snd_data_type()) {
  ...
  #### Creating the plot with draw vertical line to the starting horizontal line from the hovered point 
   drawPlot_3rd<- function(d, inputData,yr, add_horizontal, genderType, educationType, errorBar){
    ...
    dblClickData <- event_data("plotly_doubleclick")
    
    if(!is.null(d) & currentLevel() == 3){
      if (d$curveNumber != 2 && d$curveNumber != 3) {
        thePlot <- thePlot 
        return(ggplotly(thePlot, tooltip = "text", width = 1000, height=histPlotHeight()) %>% event_register("plotly_click"))
      }
    ...
    plts  <- ggplotly(thePlot,tooltip = "text", width = 1000, height=histPlotHeight()) %>% event_register("plotly_click")
    plts
  }
...
page1out <- reactive({
       ...
       if (input$educationSub == "total") {
         ...
         if (input$percentile == "All Groups") {
           ...
           text <-
            ...
            two median annual earning groups, from 1990 to 2019.
            Their median annual earnings are",
               profdif,
               "and",
               doctdif,
               "higher than those of similarly educated females in 2019. Females with uncompleted high school degrees or females who have less than 9th grade are the two lowest median annual earning groups, from 1990 to 2019. Their median annual earnings are" ,
               NotComDif,
               "and",
               LessDif,
  ...
  output$warningMessage <- renderText({
    result <- "[Click the dots in the lines for more details and double click anywhere else to exit]"
    result
  })
...
observeEvent(input$sndP_errorBar, { 
  ...
  responsiveText <- reactive({
    d <- event_data("plotly_click") 
    d_doubleClick <- event_data("plotly_doubleclick")
    print("double click data")
    print(d)
    print(d_doubleClick)
    print("double click data finished")
    ##print("hello")
    groupCount = 2
    ...
    if (!is.null(d) ) {
      ...
      if (inputX < input$tgp_range[1] | inputX > input$tgp_range[2]) {
        return(result)  
      }
  ...
  myModal <- function(){
    showModal(modalDialog(
      ...
      p(class ="modalText",
        ...
        br(),
        br(),
        tags$span(style ="color:blue !important;font-weight:bold;","If you meet any problems in the website, try double click the empty place on the plot, or refresh the page. "),
        br(),
        br(),
  ...
  output$scatterPlot_sndlv2Page <- renderPlotly({
    ...
    event <- d
    if (!is.null(d)) {
      if (d$x > input$snd_range[2] | d$x < input$snd_range[1]) {
        event <- NULL
      }
    }
  ...
  output$sgGroupHist <- renderPlotly({
    ggplotly(drawHist_2nd(), width=histBarPlotWidth_3rd(), height = histPlotHeight(), tooltip="text")  %>%  event_register("plotly_click")
  })
  ...
  responsiveText_2 <- reactive({
    ...
    if (!is.null(d) ) {
      ...
      if (clickedYear > curMaxYear | clickedYear < curMinYear) {
        return(result)
      }
      choosenY <- d["y"]
